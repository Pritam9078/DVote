...
class Web3ProviderManager {
  ...
  init() {
    ...
    try {
      ...
      console.warn('[DVote] Failed to initialize Web3 provider manager:', error.message);
    }
  }
    ...
    Object.defineProperty = function(obj, prop, descriptor) {
      if (prop === 'ethereum' && obj === window) {
        ...
        if (existingDescriptor && existingDescriptor.configurable === false) {
          ...
          console.log('[DVote] Prevented redefinition of non-configurable ethereum property');
          return obj;
        }
      }
      ...
      if ((prop === 'web3' || prop === 'solana') && obj === window) {
        ...
        if (existingDescriptor && existingDescriptor.configurable === false) {
          // Silently ignore attempts to redefine non-configurable properties
          console.log(`[DVote] Prevented redefinition of non-configurable ${prop} property`);
          return obj;
        }
      }
      ...
      try {
        ...
        if (error.message.includes('Cannot redefine property')) {
          console.log(`[DVote] Property redefinition prevented: ${prop}`);
          return obj;
        }
        throw error;
      }
  ...
  setupProviderListeners() {
    ...
    this.activeProvider.on?.('accountsChanged', (accounts) => {
      console.log('[DVote] Accounts changed:', accounts.length > 0 ? 'Connected' : 'Disconnected');
    });

    // Handle chain changes
    this.activeProvider.on?.('chainChanged', (chainId) => {
      console.log('[DVote] Chain changed:', chainId);
    });

    // Handle connection
    this.activeProvider.on?.('connect', (connectInfo) => {
      console.log('[DVote] Wallet connected:', connectInfo);
    });

    // Handle disconnection
    this.activeProvider.on?.('disconnect', (error) => {
      console.log('[DVote] Wallet disconnected:', error?.message || 'Unknown reason');
    });
  }
