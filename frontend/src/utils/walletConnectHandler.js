  ...
  interceptFetch() {
    window.fetch = async (...args) => {
      ...
      try {
        ...
        if (this.isWalletConnectUrl(url)) {
          console.log('[DVote] WalletConnect request failed, using fallback');
  ...
  async handleWalletConnectRequest(url, options) {
    try {
      ...
      if (response.status === 403) {
        console.log('[DVote] WalletConnect 403 error handled gracefully');
        return this.createFallbackResponse();
      }

      return response;
    } catch (error) {
      console.log('[DVote] WalletConnect network error handled:', error.message);
      return this.createFallbackResponse();
    }
  }
    ...
    window.WebSocket = class extends OriginalWebSocket {
      constructor(url, protocols) {
        ...
        if (typeof url === 'string' && url.includes('localhost:3001')) {
          console.log('[DVote] Blocking WebSocket connection to localhost:3001 - pure dApp mode');
        ...
        if (typeof url === 'string' && url.includes('walletconnect')) {
          this.addEventListener('error', (event) => {
            console.log('[DVote] WalletConnect WebSocket error suppressed');
            event.stopPropagation();
          });
          
          this.addEventListener('close', (event) => {
            if (event.code !== 1000) {
              console.log('[DVote] WalletConnect WebSocket closed unexpectedly, this is normal');
            }
          });
        }
      }
  ...
  configureWalletConnect(projectId) {
    try {
      if (!projectId || projectId === 'YOUR_PROJECT_ID') {
        console.warn('[DVote] WalletConnect project ID not configured');
        return null;
      }
      ...
      return {
        ...
        onError: (error) => {
          console.log('[DVote] WalletConnect error handled:', error.message);
        },
        onDisconnect: () => {
          console.log('[DVote] WalletConnect disconnected');
        }
      };
    } catch (error) {
