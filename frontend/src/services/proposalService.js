...
export class ProposalService {
  ...
  async fetchProposals() {
    try {
      ...
      // Use fetch to call local RPC directly
      const proposalCountResponse = await fetch(this.localRpcUrl, {
        method: 'POST',
        headers: {
      ...
      for (let i = 1; i <= proposalCount; i++) {
        try {
          ...
          const proposalResponse = await fetch(this.localRpcUrl, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          // Get proposal state
          const stateResponse = await fetch(this.localRpcUrl, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
